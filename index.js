...
app.post('/chat', async (req, res) => {
  ...
  if (!req.body) {
    ...
    return res.status(400).send("Chat body tidak ada nih!");
  }
  ...
  if (!messages || !Array.isArray(messages)) {
    return res.status(400).send(
      "Pesan harus berupa array berisi role dan juga konten ! Tidak ada pesan atau pesannya nggak beraturan ! "
    );
  }
  ...
  try {
    const aiResponse = await ai.models.generateContent({
      config: {
        systemInstruction: {
          parts: [
            { text: "terima kasih atas chat anda ! " }
          ]
        }
      },
...
app.listen(PORT, () => {
  console.log("Server Jadul Intel Prosesor : " + PORT);
});